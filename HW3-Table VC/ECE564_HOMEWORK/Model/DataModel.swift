...
class DukePerson: Person, BlueDevil, CustomStringConvertible {
    ...
    var description: String {
        let (pronoun, courseRole) = getGenderAndRole(self.gender, self.role)
        var langStr: String = ""
        ...
        let personalInfo: String = "\(self.fullName) is from \(self.whereFrom) and is a \(courseRole) of ECE 564. \(pronoun) is proficient in \(langStr). When not in class, \(self.firstName) enjoys \(hobbyStr).\n"
        return personalInfo
    }
