...
class TableViewController: UITableViewController, PassDataBack {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        let downloadButton = UIBarButtonItem(title: "Download", style: .plain, target: self, action: #selector(handleDownload))
        downloadButton.tintColor = UIColor.white
        downloadButton.setTitleTextAttributes([NSAttributedString.Key.font : UIFont.systemFont(ofSize: 10)], for: .normal)
        let uploadButton = UIBarButtonItem(title: "Upload", style: .plain, target: self, action: #selector(handleUpload))
        uploadButton.setTitleTextAttributes([NSAttributedString.Key.font : UIFont.systemFont(ofSize: 10)], for: .normal)
...
//        print(teamsDataFilePath)
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "showInformation" {
            ...
            informationVC.peopleWithDrawingAnimations = self.peopleWithDrawingAnimations
    ...
    func assignTeams() {
        ...
        for person in self.people {
            if self.teams[person.team] == nil && person.role == .Student { // new team
                self.teams[person.team] = teamNum
                teamNum += 1
            }
        }
    }
    
    func generatePeopleArray() {
...
//MARK: - ServiceDownloadDelegate
extension TableViewController: ServiceDownloadDelegate {
    fileprivate func prepTableDataSource(dukePersons: [DukePerson]) {
        // people
        self.people = dukePersons
        savePeopleData()
        // teams
        assignTeams()
        saveTeamsData()
    ...
    func didFinishDownload(dukePersons: [DukePerson]) {
        prepTableDataSource(dukePersons: dukePersons)
        tableView.reloadData()
    }
}
